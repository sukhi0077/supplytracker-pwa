...
var Serializer = class {
  ...
  _encodeUserBroadcastPush(message, encodingType, encodedPayload) {
    ...
    const ref2 = encoder.encode((_a = message.ref) !== null && _a !== void 0 ? _a : "");
...
var serializer_default = {
  ...
  */
  ...
  binaryEncode(message) {
    ...
    let refBytes = encoder.encode(ref2);
  ...
  triggerChanError(reason) {
    this.channels.forEach((channel2) => {
      if (!(channel2.isErrored() || channel2.isLeaving() || channel2.isClosed())) {
        channel2.trigger(CHANNEL_EVENTS2.error, reason);
      }
    });
  }
  /**
  ...
  subscribe(callback, timeout = this.timeout) {
    ...
    if (this.channelAdapter.isClosed()) {
      ...
      this.channelAdapter.subscribe(timeout).receive("ok", async ({ postgres_changes: postgres_changes2 }) => {
        ...
        const message = Object.values(error3).join(", ") || "error";
        callback === null || callback === void 0 ? void 0 : callback(REALTIME_SUBSCRIBE_STATES.CHANNEL_ERROR, new Error(message, { cause: error3 }));
      }).receive("timeout", () => {
...
var handleError = /* @__PURE__ */ __name(async (error3, reject, options, namespace) => {
  if (error3 !== null && typeof error3 === "object" && "json" in error3 && typeof error3.json === "function") {
...
function isAuthRefreshDiscardedError(error3) {
  return isAuthError(error3) && error3.name === "AuthRefreshDiscardedError";
}
...
var GoTrueClient = class _GoTrueClient {
    ...
    try {
      ...
      if (expiresWithMargin) {
        if (this.autoRefreshToken && currentSession.refresh_token) {
          ...
          if (error3) {
            if (isAuthRefreshDiscardedError(error3)) {
              this._debug(debugName, "refresh discarded by commit guard", error3);
            } else {
              this._debug(debugName, "refresh failed", error3);
            }
          }
        }
  ...
  async _startPasskeyRegistration() {
    ...
    try {
      return await this._useSession(async (result) => {
        ...
        const { data, error: error3 } = await _request(this.fetch, "POST", `${this.url}/passkeys/registration/options`, {
          headers: this.headers,
          jwt: session.access_token,
          body: {}
        });
  ...
  async _verifyPasskeyRegistration(params) {
    ...
    try {
      return await this._useSession(async (result) => {
        ...
        const { data, error: error3 } = await _request(this.fetch, "POST", `${this.url}/passkeys/registration/verify`, {
          headers: this.headers,
          jwt: session.access_token,
          body: {
  ...
  async _startPasskeyAuthentication(params) {
    ...
    assertPasskeyExperimentalEnabled(this.experimental);
    try {
      const { data, error: error3 } = await _request(this.fetch, "POST", `${this.url}/passkeys/authentication/options`, {
        headers: this.headers,
        body: {
  ...
  async _verifyPasskeyAuthentication(params) {
    assertPasskeyExperimentalEnabled(this.experimental);
    try {
      const { data, error: error3 } = await _request(this.fetch, "POST", `${this.url}/passkeys/authentication/verify`, {
        headers: this.headers,
        body: {
          challenge_id: params.challengeId,
          credential: params.credential
  ...
  async _listPasskeys() {
    ...
    try {
      return await this._useSession(async (result) => {
        ...
        const { data, error: error3 } = await _request(this.fetch, "GET", `${this.url}/passkeys`, {
          headers: this.headers,
          jwt: session.access_token,
  ...
  async _updatePasskey(params) {
    ...
    try {
      return await this._useSession(async (result) => {
        ...
        const { data, error: error3 } = await _request(this.fetch, "PATCH", `${this.url}/passkeys/${params.passkeyId}`, {
          headers: this.headers,
          jwt: session.access_token,
  ...
  async _deletePasskey(params) {
    ...
    try {
      return await this._useSession(async (result) => {
        ...
        const { error: error3 } = await _request(this.fetch, "DELETE", `${this.url}/passkeys/${params.passkeyId}`, {
          headers: this.headers,
          jwt: session.access_token,
          noResolveJson: true
        });
...
if (typeof Deno !== "undefined") {
  ...
  JS_ENV = "node";
  const _process2 = globalThis["process"];
  JS_RUNTIME_VERSION = _process2 === null || _process2 === void 0 || (_process$version = _process2["version"]) === null || _process$version === void 0 ? void 0 : _process$version.replace(/^v/, "");
}
var _Deno$version;
...
function parseTraceParent(traceparent) {
  ...
  const hexRegex2 = /^[0-9a-f]+$/i;
  if (!hexRegex2.test(version$1) || !hexRegex2.test(traceId) || !hexRegex2.test(parentId) || !hexRegex2.test(traceFlags)) return null;
...
function matchStringTarget(hostname, target) {
  ...
  if (target.startsWith("*.")) {
    ...
    if (hostname.endsWith(domain2)) {
      if (hostname === domain2 || hostname.endsWith("." + domain2)) return true;
    }
  }
  return false;
}
...
function extractSpkiFromCert(der) {
  ...
  if (!tbs || tbs.tag !== 48) throw new Error("Bad certificate: no tbsCertificate");
...
__name(extractSpkiFromCert, "extractSpkiFromCert");

// src/ksef/client.ts
var sleep3 = /* @__PURE__ */ __name((ms) => new Promise((r) => setTimeout(r, ms)), "sleep");
...
var KsefClient = class {
  ...
  async req(method, path, opts = {}) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      if (resp.status === 429 && attempt === 0) {
        ...
        await sleep3((Number.isFinite(retry) ? retry : 5) * 1e3);
        continue;
      }
  ...
  async resolveEncryptKey() {
    ...
    if (this.cfg.publicKeyPem) {
      ...
      const items = Array.isArray(data) ? data : data.publicKeyCertificates || data.certificates || data.items || [];
      let certB64;
      for (const raw of items) {
        const c = raw;
        let usage = c.usage ?? c.usages ?? [];
        if (typeof usage === "string") usage = [usage];
        if (Array.isArray(usage) && usage.includes("KsefTokenEncryption")) {
          certB64 = c.certificate || c.publicKeyCertificate || c.value;
          if (certB64) break;
        }
      }
  ...
  async openSession() {
    ...
    const tsMillis = typeof challengeResp.timestamp === "number" ? challengeResp.timestamp : Date.parse(challengeResp.timestamp) || Date.now();
    ...
    const referenceNumber = init.referenceNumber || init.referenceNo;
    const authenticationToken = init.authenticationToken?.token || init.authenticationToken || init.token;
    ...
    const accessToken = redeem.accessToken?.token || redeem.accessToken || redeem.access_token;
    const refreshToken = redeem.refreshToken?.token || redeem.refreshToken || redeem.refresh_token || "";
  ...
  async queryInvoices(dateFrom, dateTo) {
    const body = {
    ...
    for (let page = 0; page < 200; page++) {
      ...
      const list = resp.invoices || resp.invoiceList || resp.items || resp.results || resp.content || [];
      for (const raw of list) {
        const inv = raw;
        const ref2 = inv.ksefNumber || inv.ksefReferenceNumber || inv.referenceNumber || "";
        if (ref2)
          out.push({
            ksefReference: ref2,
  ...
  async fetchInvoiceXml(ksefReference) {
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
      ...
      const wait = Number.isFinite(hinted) && hinted > 0 ? hinted * 1e3 : 3e3 * 2 ** attempt;
...
var EntityDecoder = class {
   ...
   */
  setXmlVersion(version6) {
    this._ncrXmlVersion = version6 === 1.1 ? 1.1 : 1;
  }
  // -------------------------------------------------------------------------
  // Primary API
  // -------------------------------------------------------------------------
...
// src/ksef/parser.ts
var KsefParseError = class extends Error {
  static {
    __name(this, "KsefParseError");
  }
};
...
function textOf(v) {
  ...
  if (typeof v === "object") {
    const t = v["#text"];
    if (t !== void 0) return textOf(t);
  }
  return null;
}
__name(textOf, "textOf");
function textLocal(node, name) {
  return textOf(findFirst(node, name));
}
__name(textLocal, "textLocal");
...
__name(toDecimalOrZero, "toDecimalOrZero");
...
__name(parseVatRate, "parseVatRate");
function parseDate(value) {
  if (!value) return null;
  const iso2 = value.replace("Z", "+00:00");
...
var round2 = /* @__PURE__ */ __name((d) => Math.round((d + Number.EPSILON) * 100) / 100, "round2");
function parseFa(xml) {
  ...
  try {
    ...
    throw new KsefParseError(`Invalid XML: ${e.message}`);
  }
  const fa = findFirst(root, "Fa");
  if (fa === void 0) throw new KsefParseError("No <Fa> element found");
  const podmiot1 = findFirst(root, "Podmiot1");
  if (podmiot1 === void 0) throw new KsefParseError("No <Podmiot1> (supplier) found");
  ...
  const supplier_nip = (dane !== void 0 ? textLocal(dane, "NIP") : "") || "";
  const supplier_name = (dane !== void 0 ? textLocal(dane, "Nazwa") : null) || textLocal(podmiot1, "Nazwa") || "";
  const wiersze = [];
  ...
  if (!net_inv && gross_inv) {
    ...
    if (rates.size <= 1) {
      const rate = rates.values().next().value ?? 0;
...
__name(fillLineGross, "fillLineGross");

// src/ksef/matching.ts
var DATE_FULL = /\b\d{1,2}[.\-/]\d{1,2}[.\-/]\d{2,4}\b|\b\d{4}[.\-/]\d{1,2}[.\-/]\d{1,2}\b/g;
var DATE_MONTH_YEAR = /\b\d{1,2}[.\-/]\d{4}\b/g;
var DASH_CONNECTOR = /\s+[-–—]\s+/g;
var WS = /\s+/g;
var CURRENCY2 = /* @__PURE__ */ new Set(["pln", "z\u0142", "zl", "eur", "usd", "gbp"]);
function tokenIsNoise(tok) {
  const core = tok.replace(/^[().,:;/–—-]+|[().,:;/–—-]+$/g, "");
  const low = core.toLowerCase();
  if (CURRENCY2.has(low)) return true;
  if (core && /\d/.test(core)) {
...
__name(tokenIsNoise, "tokenIsNoise");
function canonicalize(name, lower) {
...
function normalizeKsefName(name) {
  if (!name) return "";
  return canonicalize(name, true);
}
__name(normalizeKsefName, "normalizeKsefName");

// src/ksef/sync.ts
var MAX_NEW_PER_RUN = 6;
var PACE_MS = 1500;
var RATE_LIMIT_GIVE_UP = 2;
var sleep4 = /* @__PURE__ */ __name((ms) => new Promise((r) => setTimeout(r, ms)), "sleep");
async function loadSuppliers(db2) {
  const { data } = await db2.from("suppliers").select("id, name, nip, ksef_name");
  const byNip = /* @__PURE__ */ new Map();
...
async function resolveSupplier(db2, maps, inv) {
  ...
  const { data: created, error: error3 } = await db2.from("suppliers").insert({ name, nip, ksef_name: legal }).select("id").single();
  if (error3) throw new Error(`create supplier: ${error3.message}`);
  const id = created.id;
...
__name(resolveSupplier, "resolveSupplier");
async function loadMappings(db2) {
  const { data } = await db2.from("ksef_mappings").select("ksef_item_name,item_id,pack_size");
  const map = /* @__PURE__ */ new Map();
  for (const m of data || []) {
    map.set(normalizeKsefName(m.ksef_item_name), {
      itemId: m.item_id,
...
__name(loadMappings, "loadMappings");
async function runKsefFetch(env2, db2, dateFrom, dateTo, opts = {}) {
  const res = { found: 0, created: 0, updated: 0, skipped: 0, remaining: 0, errors: [] };
  const { data: job } = await db2.from("ksef_fetch_jobs").insert({
    status: "running",
    environment: opts.environment || env2.KSEF_ENV,
    date_from: dateFrom,
    date_to: dateTo
  }).select("id").single();
  const jobId = job?.id;
  const finish = /* @__PURE__ */ __name(async (status, note = "") => {
    if (!jobId) return;
    await db2.from("ksef_fetch_jobs").update({
      status,
  ...
  try {
    const client = new KsefClient({
      baseUrl: opts.baseUrl || env2.KSEF_BASE_URL,
      nip: opts.creds?.nip || env2.KSEF_NIP,
      token: opts.creds?.token || env2.KSEF_TOKEN,
      publicKeyPem: env2.KSEF_PUBLIC_KEY_PEM || void 0
    });
    ...
    const staleness = /* @__PURE__ */ new Map();
    if (refList.length) {
      const { data: existing } = await db2.from("invoices").select("id, ksef_reference, updated_at").in("ksef_reference", refList);
      for (const e of existing || []) {
        existingMap.set(e.ksef_reference, e.id);
        staleness.set(e.ksef_reference, Date.parse(String(e.updated_at || "")) || 0);
      }
    }
    const suppliers = await loadSuppliers(db2);
    const mappings = await loadMappings(db2);
    const writeStock = String(env2.KSEF_WRITE_STOCK).toLowerCase() === "true";
    ...
    for (const ref2 of candidates) {
      if (processed >= MAX_NEW_PER_RUN || rateLimitStreak >= RATE_LIMIT_GIVE_UP) {
        res.remaining++;
        continue;
      }
      if (processed > 0) await sleep4(PACE_MS);
      processed++;
      try {
        const xml = await client.fetchInvoiceXml(ref2.ksefReference);
        rateLimitStreak = 0;
        const inv = parseFa(xml);
        const supplierId = await resolveSupplier(db2, suppliers, inv);
        const r2 = /* @__PURE__ */ __name((v) => round(v ?? null, 2), "r2");
        const header = {
          supplier_id: supplierId,
          number: inv.number,
          ksef_reference: ref2.ksefReference,
        ...
        const existingId = existingMap.get(ref2.ksefReference);
        let invoiceId;
        if (existingId) {
          invoiceId = existingId;
          await db2.from("invoices").update(header).eq("id", invoiceId);
          await db2.from("invoice_lines").delete().eq("invoice_id", invoiceId);
          res.updated++;
        } else {
          const { data: ins, error: error3 } = await db2.from("invoices").insert(header).select("id").single();
        ...
        if (lineRows.length) await db2.from("invoice_lines").insert(lineRows);
        if (writeStock) {
          const moves = lineRows.filter((r) => r.item_id).map((r) => ({
            item_id: r.item_id,
          ...
          if (moves.length) await db2.from("stock_movements").insert(moves);
        }
      } catch (e) {
        const msg = e.message;
        if (/-> (429|503)\b/.test(msg)) {
          ...
          res.errors.push(`${ref2.ksefReference}: ${msg}`);
        }
      }
    }
    const note = res.remaining > 0 ? rateLimitStreak >= RATE_LIMIT_GIVE_UP ? `KSeF rate limit hit \u2014 ${res.remaining} left.` : opts.updateExisting && res.created === 0 ? `All ${res.found} already imported \u2014 refreshed ${res.updated}; untick "Update existing" for new only.` : `${res.remaining} left (per-run cap).` : "";
    res.note = note;
    await finish(res.errors.length ? "partial" : "success", note);
  } catch (e) {
    res.errors.push(e.message);
    const host = String(opts.baseUrl || env2.KSEF_BASE_URL || "(no base url)").replace(/^https?:\/\//, "").split("/")[0];
    const nip = opts.creds?.nip || env2.KSEF_NIP;
    await finish(
      "failed",
      `Failed against ${host} (env ${opts.environment || env2.KSEF_ENV || "?"}), NIP ${nip ? "set" : "MISSING"}, token ${opts.creds?.token || env2.KSEF_TOKEN ? "set" : "MISSING"}.`
    );
  }
  return res;
}
__name(runKsefFetch, "runKsefFetch");

// src/wfirma/client.ts
var BASE_URL = "https://api2.wfirma.pl";
var WfirmaClient = class {
  constructor(cfg) {
    this.cfg = cfg;
  }
  cfg;
  static {
    __name(this, "WfirmaClient");
  }
  authHeader() {
    return "Basic " + btoa(`${this.cfg.login}:${this.cfg.password}`);
  }
  async request(module, action, body) {
...
function extractRecords(data, plural, singular) {
  const top = data.invoices || data[plural];
  if (!top) return [];
  const out = [];
  for (const [key, val] of Object.entries(top)) {
    if (key === "parameters") continue;
    const rec = val?.[singular] ?? val;
...
// src/wfirma/sync.ts
var normNumber = /* @__PURE__ */ __name((s) => String(s ?? "").replace(/\s+/g, "").toUpperCase(), "normNumber");
var toNum = /* @__PURE__ */ __name((v) => {
  if (v === null || v === void 0 || v === "") return null;
  const n = Number(String(v).replace(/\s+/g, "").replace(",", "."));
  return Number.isFinite(n) ? n : null;
}, "toNum");
var pick = /* @__PURE__ */ __name((rec, ...keys) => {
  for (const k of keys) if (rec[k] !== void 0 && rec[k] !== null && rec[k] !== "") return rec[k];
  return void 0;
}, "pick");
function derive(rec) {
...
async function runWfirmaSync(env2, db2, opts = {}) {
  const res = { expensesSeen: 0, matched: 0, updated: 0, unmatchedLocal: 0, errors: [] };
  const client = new WfirmaClient({ login: env2.WFIRMA_LOGIN, password: env2.WFIRMA_PASSWORD });
  ...
  let q = db2.from("invoices").select("id,number,issue_date");
  if (opts.dateFrom) q = q.gte("issue_date", opts.dateFrom);
  if (opts.dateTo) q = q.lte("issue_date", opts.dateTo);
  const { data: invoices, error: error3 } = await q;
  if (error3) {
    res.errors.push(error3.message);
    return res;
  }
  ...
  for (const inv of invoices || []) {
    ...
    const { error: upErr } = await db2.from("invoices").update({
      wfirma_id: wid,
...
__name(runWfirmaSync, "runWfirmaSync");

// src/lib/auth.ts
async function verifyAdmin(req, env2, db2) {
  const header = req.headers.get("Authorization") || "";
  const jwt = header.startsWith("Bearer ") ? header.slice(7) : "";
  if (!jwt) throw new Error("Not signed in");
  const resp = await fetch(`${env2.SUPABASE_URL}/auth/v1/user`, {
    headers: { apikey: env2.SUPABASE_SERVICE_ROLE_KEY, Authorization: `Bearer ${jwt}` }
  });
  if (!resp.ok) throw new Error("Invalid or expired session");
  const user = await resp.json();
  const { data, error: error3 } = await db2.from("profiles").select("role").eq("id", user.id).maybeSingle();
  if (error3) throw new Error(error3.message);
...
var daysAgo = /* @__PURE__ */ __name((n) => iso(new Date(Date.now() - n * 864e5)), "daysAgo");
var KSEF_HOSTS = {
  test: "https://api-test.ksef.mf.gov.pl/api/v2",
  prod: "https://api.ksef.mf.gov.pl/api/v2"
};
var baseUrlFor = /* @__PURE__ */ __name((env2, environment) => environment && KSEF_HOSTS[environment] || env2.KSEF_BASE_URL, "baseUrlFor");
var CORS = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Methods": "POST, OPTIONS",
  "Access-Control-Allow-Headers": "authorization, content-type, x-trigger-secret"
};
var jsonRes = /* @__PURE__ */ __name((obj, status = 200) => new Response(JSON.stringify(obj), { status, headers: { "content-type": "application/json", ...CORS } }), "jsonRes");
var index_default = {
  // --- Cron ---
  async scheduled(event, env2, ctx) {
    const client = db(env2);
    const run = /* @__PURE__ */ __name((label, p) => ctx.waitUntil(
      p.then(
        () => {
        },
        (e) => console.error(`[cron ${label}] ${e?.stack || e}`)
      )
    ), "run");
    if (event.cron === "40 6 * * *") {
      run("wfirma", runWfirmaSync(env2, client, { dateFrom: daysAgo(40), dateTo: iso(/* @__PURE__ */ new Date()) }));
    } else {
      run("ksef", runKsefFetch(env2, client, daysAgo(3), iso(/* @__PURE__ */ new Date()), { updateExisting: true }));
    }
  },
  ...
  async fetch(req, env2) {
    ...
    try {
      ...
      const isAction = req.method === "POST" && (url.pathname.startsWith("/run/") || url.pathname === "/auth-test");
      if (!isAction) {
      ...
      if (env2.TRIGGER_SECRET && req.headers.get("x-trigger-secret") === env2.TRIGGER_SECRET) authorized = true;
      if (!authorized) {
        try {
          await verifyAdmin(req, env2, client);
          authorized = true;
        } catch {
          authorized = false;
        }
      ...
      const creds = body.nip && body.token ? { nip: String(body.nip), token: String(body.token) } : void 0;
      const environment = body.environment ? String(body.environment) : void 0;
      const baseUrl = baseUrlFor(env2, environment);
      const from = url.searchParams.get("from") || daysAgo(7);
      const to = url.searchParams.get("to") || iso(/* @__PURE__ */ new Date());
      try {
        if (url.pathname === "/auth-test") {
          const k = new KsefClient({
            baseUrl,
            nip: creds?.nip || env2.KSEF_NIP,
            token: creds?.token || env2.KSEF_TOKEN,
            publicKeyPem: env2.KSEF_PUBLIC_KEY_PEM || void 0
          });
        ...
        if (url.pathname === "/run/ksef") {
          const r = await runKsefFetch(env2, client, from, to, {
        ...
        if (url.pathname === "/run/wfirma") {
          const r = await runWfirmaSync(env2, client, { dateFrom: from, dateTo: to });
          return jsonRes(r);
        }
      ...
      return jsonRes({ error: `Worker error: ${e.message}` }, 500);
    }
  }
};
export {
